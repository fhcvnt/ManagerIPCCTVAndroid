...
import lacty.cctv.manageripcctv.databinding.ActivitySqliteLocationBinding;

public class SQLiteLocation extends AppCompatActivity {
    ActivitySqliteLocationBinding binding;
    Database database;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sqlite_location);

        binding = ActivitySqliteLocationBinding.inflate(getLayoutInflater());
    ...
    public void sqlSearch(String chuoi) {
        ...
        try {
            // so thu tu
            int stt = 1;
            String select = "SELECT MaLocation,Location FROM Location WHERE Location LIKE '%" + chuoi + "%' ORDER BY Location ASC";
            ...
            while (result.moveToNext()) {
                ...
                binding.tableSQLiteLocation.addView(tr, new TableLayout.LayoutParams(TableRow.LayoutParams.MATCH_PARENT, TableRow.LayoutParams.WRAP_CONTENT));
                stt++;
            }
            result.close();
            database.close();
    ...
    public void clearTable() {
        int countrow = binding.tableSQLiteLocation.getChildCount();
        binding.tableSQLiteLocation.removeViews(1, countrow - 1);
    }
}